...
export default function CorrectScreenScience(props) {
    ...
    const PScoreScience= route.params.data1
    return (
  ...
  backgroundColor:"#013220"}} ><Text style={{color:"white", textAlign:"center", fontWeight: 'bold', fontSize:40}} onPress= {()=>{props.navigation.navigate(PScreenNameScience, {data1:PScoreScience})}} >Next -{'>'}</Text></TouchableOpacity>
  <Text>{PScreenNameScience}</Text>
  <Text>{PScoreScience}</Text>
        </View>
    )
}
